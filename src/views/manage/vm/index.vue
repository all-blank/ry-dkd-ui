<template>
  <div class="app-container">
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="vmRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="设备编号">
          <el-input :value="form.innerCode || '系统自动生成'" disabled />
        </el-form-item>
        <el-form-item label="供货时间" v-if="form.innerCode != null">
          <el-input :value="parseTime(form.lastSupplyTime, '{y}-{m}-{d} {h}:{i}:{s}')" disabled />
        </el-form-item>
        <el-form-item label="设备类型" v-if="form.innerCode != null">
          <div v-for="item in vmTypeList" :key="item.id">
            <el-input :value="item.name" v-if="form.vmTypeId == item.id" disabled />
          </div>
        </el-form-item>
        <el-form-item label="设备容量" v-if="form.innerCode != null">
          <el-input :value="form.channelMaxCapacity" disabled />
        </el-form-item>
        <!-- 增加框 -->
        ...
        <el-form-item label="所在点位" prop="nodeId">
        ...
        <el-form-item label="合作商" v-if="form.innerCode != null">
          <div v-for="item in partnerList" :key="item.id">
            <el-input :value="item.partnerName" v-if="form.partnerId == item.id" disabled />
          </div>
        </el-form-item>
        <el-form-item label="所属区域" v-if="form.innerCode != null">
          <div v-for="item in regionList" :key="item.id">
            <el-input :value="item.regionName" v-if="form.regionId == item.id" disabled />
          </div>
        </el-form-item>
        <el-form-item label="设备地址" v-if="form.innerCode != null">
          <el-input :value="form.addr" disabled />
        </el-form-item>
    ...
    <!-- 策略管理对话框 -->
    <el-dialog :title="title" v-model="openVmPolicyInfo" width="500px" append-to-body>
    ...
    if (valid) {
      if (form.value.id != null) {
        updateVm(form.value).then(response => {
          ...
          open.value = false;
          openVmPolicyInfo.value = false; // 关闭策略管理对话框
          getList();
        });
      } else {
...
// ********************货道********************
// 货道组件
import ChannelDialog from '@/components/ChannelDialog/ChannelDialog.vue';
...
/** 设备策略 */
const openVmPolicyInfo = ref(false);
